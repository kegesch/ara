...
import type { Entity, Edge, EdgeType, AradGraph } from '../types';
...
export function getDependents(g: AradGraph, id: string): Entity[] {
  ...
  for (const edge of incoming) {
    ...
    if (['driven_by', 'derived_from', 'enables'].includes(edge.type)) {
...
export function findDanglingRefs(g: AradGraph): { from: string; ref: string; context: string }[] {
  ...
  for (const edge of g.edges) {
    ...
    if (!g.entities.has(edge.from)) {
      danglers.push({ from: edge.to, ref: edge.from, context: `${edge.type} (reverse)` });
    }
  }
  return danglers;
}
 ...
 */
export function impactAnalysis(g: AradGraph, id: string): {
  direct: Entity[];
  transitive: Entity[];
} {
...
export function traceUp(g: AradGraph, id: string, visited: Set<string> = new Set()): TraceNode | null {
  ...
  for (const edge of outgoing) {
    if (['driven_by', 'derived_from', 'supersedes'].includes(edge.type)) {
...
export function findPossibleContradictions(
  ...
  const requirements = [...g.entities.values()].filter(e => e.type === 'requirement');
 ...
 */
export function findPossibleDuplicates(g: AradGraph, threshold: number = 0.6): PossibleDuplicate[] {
  const requirements = [...g.entities.values()].filter(e => e.type === 'requirement');
  ...
  for (const [, entity] of g.entities) {
    if (entity.type === 'decision' && entity.status === 'accepted') {
      ...
      for (const drivenById of entity.driven_by) {
        ...
        if (dep.type === 'requirement' && (dep.status === 'deprecated' || dep.status === 'rejected')) {
          badRefs.push(dep);
        }
