 ...
 */
export function searchEntities(entities: Entity[], query: string): SearchResult[] {
...
function tokenize(query: string): Token[] {
  return query
    ...
    .map(part => {
      ...
      if (modifierMatch) {
        return { type: 'modifier' as const, key: modifierMatch[1].toLowerCase(), value: modifierMatch[2].toLowerCase() };
      }
      return { type: 'text' as const, value: part.toLowerCase() };
    });
}

function scoreEntity(entity: Entity, tokens: Token[]): { score: number; matches: string[] } {
  let totalScore = 0;
  ...
  if (hasTextTokens && !matches.some(m => !m.includes(':'))) return { score: 0, matches: [] };
...
function scoreModifier(entity: Entity, key: string, value: string): number {
  switch (key) {
    ...
    case 'tag':
      return entity.tags.some(t => t.toLowerCase() === value || t.toLowerCase().includes(value)) ? 30 : 0;
    case 'driven_by':
    case 'derived_from':
    case 'enables':
    case 'conflicts_with':
    ...
    case 'id':
      return entity.id.toLowerCase() === value ? 100 : entity.id.toLowerCase().includes(value) ? 50 : 0;
    case 'promoted_to':
      return entity.type === 'assumption' && entity.promoted_to?.toLowerCase().includes(value) ? 35 : 0;
    default:
      return 0;
  }
}

function hasRelationship(entity: Entity, relType: string, value: string): boolean {
  switch (entity.type) {
    case 'requirement':
      if (relType === 'derived_from') return entity.derived_from.some(d => d.toLowerCase().includes(value));
      if (relType === 'conflicts_with') return entity.conflicts_with.some(d => d.toLowerCase().includes(value));
      return false;
    case 'decision':
      if (relType === 'driven_by') return entity.driven_by.some(d => d.toLowerCase().includes(value));
      if (relType === 'enables') return entity.enables.some(d => d.toLowerCase().includes(value));
      return false;
    default:
      return false;
  }
