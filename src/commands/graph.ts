...
export function renderMermaid(graph: {
	...
	// Group nodes by type
	const requirements = [...graph.entities.values()].filter(
		(e) => e.type === "requirement",
	);
	const assumptions = [...graph.entities.values()].filter(
		(e) => e.type === "assumption",
	);
	const decisions = [...graph.entities.values()].filter(
		(e) => e.type === "decision",
	);
	const ideas = [...graph.entities.values()].filter((e) => e.type === "idea");
	...
	if (requirements.length > 0) {
		...
		for (const r of requirements) {
			lines.push(
				`    ${r.id}["${r.id}: ${escMermaid(r.title)}<br/><small>${r.status}</small>"]`,
			);
		}
		lines.push("  end");
	}
	...
	if (assumptions.length > 0) {
		...
		for (const a of assumptions) {
			lines.push(
				`    ${a.id}["${a.id}: ${escMermaid(a.title)}<br/><small>${a.status}</small>"]`,
			);
		}
		lines.push("  end");
	}
	...
	if (decisions.length > 0) {
		...
		for (const d of decisions) {
			lines.push(
				`    ${d.id}["${d.id}: ${escMermaid(d.title)}<br/><small>${d.status}</small>"]`,
			);
		}
		lines.push("  end");
	}
	...
	if (ideas.length > 0) {
		lines.push("  subgraph Ideas");
		for (const i of ideas) {
			lines.push(
				`    ${i.id}["${i.id}: ${escMermaid(i.title)}<br/><small>${i.status}</small>"]`,
			);
		}
		lines.push("  end");
	}
	...
	lines.push(
		`  classDef idea fill:#dda0dd,stroke:#333,color:#333,stroke-dasharray: 5 5`,
	);
	...
	for (const i of ideas) lines.push(`  class ${i.id} idea`);
	...
	for (const edge of graph.edges) {
		...
		switch (edge.type) {
			...
			case "promoted_to":
				...
				break;
			case "inspired_by":
				lines.push(`  ${edge.from} -.->|"inspired by"| ${edge.to}`);
				break;
		}
	}
...
export function renderDot(graph: {
	...
	for (const edge of graph.edges) {
		...
		switch (edge.type) {
			...
			case "supersedes":
				lines.push(
					`  "${edge.from}" -> "${edge.to}" [label="supersedes", style=dashed];`,
				);
				break;
			case "conflicts_with":
				lines.push(
					`  "${edge.from}" -> "${edge.to}" [label="conflicts", color=red, style=bold];`,
				);
				break;
			case "promoted_to":
				lines.push(
					`  "${edge.from}" -> "${edge.to}" [label="promoted", style=dotted];`,
				);
				break;
			case "inspired_by":
				lines.push(
					`  "${edge.from}" -> "${edge.to}" [label="inspired by", style=dotted];`,
				);
				break;
		}
	}
...
function renderAscii(graph: {
	...
	const rootReqs = requirements.filter(
		(r) => r.type === "requirement" && (r as any).derived_from.length === 0,
	);
	const assumptions = [...graph.entities.values()].filter(
		(e) => e.type === "assumption",
	);
	...
	for (const root of rootReqs) {
		lines.push(
			`${statusSym(root.status)} ${root.id} ${root.title} [${root.type}]`,
		);
	...
	// Print ideas
	for (const i of ideas) {
		lines.push(`${statusSym(i.status)} ${i.id} ${i.title} [${i.type}]`);
		const visited = new Set<string>([i.id]);
		renderAsciiChildren(graph, i.id, "  ", visited, lines);
		lines.push("");
	}
...
function renderAsciiChildren(
	...
	for (const edge of incoming) {
		if (["driven_by", "enables", "inspired_by"].includes(edge.type)) {
	...
	for (const [childId, edgeType] of dependents) {
		...
		lines.push(
			`${prefix}${connector}${statusSym(child.status)} ${childId} ${child.title} [${edgeType}]`,
		);
