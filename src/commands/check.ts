...
import {
  buildGraph, findOrphans, findContradictions, findDanglingRefs, findUnvalidatedAssumptions,
  findPossibleContradictions, findPossibleDuplicates, findStatusAnomalies, findOrphanRequirements,
} from '../graph/graph.js';
import { formatEntityBrief, colorId, red, yellow, green, bold, dim } from '../display/format.js';
...
export function runCheck(): CheckResult {
  ...
  for (const a of findUnvalidatedAssumptions(graph)) {
    ...
    const depCount = incoming.filter(e => ['driven_by', 'enables'].includes(e.type)).length;
...
function displayText(result: CheckResult, strict: boolean): void {
  ...
  console.log(dim(`  ${result.entities} entities, ${result.relationships} relationships`));
  console.log('');
