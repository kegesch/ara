...
import type { Entity, Decision, Requirement } from '../types.js';
import { colorId, red, green, yellow, bold, dim } from '../display/format.js';

export function removeCommand(id: string, options?: { force?: boolean }): void {
  requireAradProject();
  ...
  if (dependents.length > 0 && !options?.force) {
    console.error(yellow(`⚠ ${colorId(id)} is referenced by ${dependents.length} other entities:`));
    for (const dep of dependents) {
      console.error(`  ${colorId(dep.id)} "${dep.title}"`);
    }
    console.error('');
    console.error(`Use --force to remove anyway (will leave dangling references).`);
    console.error(`Or use --clean to also remove references from dependent entities.`);
    process.exit(1);
  }
