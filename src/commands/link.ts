...
import { readAllEntities, requireAradProject, readEntityById, updateEntity } from '../io/files.js';
import type { Entity, EntityType, EdgeType, Decision, Requirement } from '../types.js';
import { getTypeFromId } from '../types.js';
import { colorId, red, green, yellow, bold, dim } from '../display/format.js';
 ...
 */
function getRelField(entity: Entity, edgeType: EdgeType): { field: string; isArray: boolean } | null {
  switch (entity.type) {
    case 'decision':
      if (edgeType === 'driven_by') return { field: 'driven_by', isArray: true };
      if (edgeType === 'enables') return { field: 'enables', isArray: true };
      if (edgeType === 'supersedes') return { field: 'supersedes', isArray: false };
      return null;
    case 'requirement':
      if (edgeType === 'derived_from') return { field: 'derived_from', isArray: true };
      if (edgeType === 'conflicts_with') return { field: 'conflicts_with', isArray: true };
      return null;
    default:
      return null;
  }
...
function inferEdgeType(fromType: EntityType, toType: EntityType): EdgeType | null {
...
export function linkCommand(fromId: string, toId: string, options?: { type?: string }): void {
  requireAradProject();
  ...
  if (options?.type) {
    ...
    if (!edgeType) {
      ...
      if (valid && valid.length > 1) {
        console.error(yellow(`Ambiguous relationship between ${fromEntity.type} → ${toEntity.type}.`));
        console.error(`  Specify --type: ${valid.join(', ')}`);
      } else {
        console.error(red(`No valid relationship from ${fromEntity.type} (${fromId}) to ${toEntity.type} (${toId}).`));
      }
      process.exit(1);
    }
  }
  ...
  if (!validForPair || !validForPair.includes(edgeType)) {
    console.error(red(`Edge type "${edgeType}" is not valid for ${fromEntity.type} → ${toEntity.type}.`));
    if (validForPair) {
  ...
  if (relField.isArray) {
    ...
    if (arr.includes(toId)) {
      console.log(yellow(`${colorId(fromId)} already has ${edgeType} → ${colorId(toId)}.`));
      return;
    }
    // Add to array
    arr.push(toId);
    ...
    if (existing) {
      console.error(yellow(`${colorId(fromId)} already has ${edgeType} → ${colorId(existing)}. Overwriting.`));
    }
  ...
  console.log(green(`✓ Linked ${colorId(fromId)} ──${edgeType}──▶ ${colorId(toId)}`));
}

export function unlinkCommand(fromId: string, toId: string, options?: { type?: string }): void {
  requireAradProject();
  ...
  for (const edgeType of edgeTypesToCheck) {
    ...
    if (relField.isArray) {
      ...
      if (idx !== -1) {
        arr.splice(idx, 1);
        removed = true;
        console.log(green(`✓ Removed ${edgeType}: ${colorId(fromId)} → ${colorId(toId)}`));
      }
    } else {
  ...
  if (toEntity && fromEntity.type === 'requirement' && toEntity.type === 'requirement') {
    ...
    if (idx !== -1) {
      ...
      updateEntity(process.cwd(), toReq);
      console.log(green(`✓ Removed reverse conflicts_with: ${colorId(toId)} → ${colorId(fromId)}`));
    }
  }

  if (!removed) {
    console.log(yellow(`No relationship found from ${colorId(fromId)} to ${colorId(toId)}.`));
    return;
  }
