...
import { readAllEntities, requireAradProject, ARAD_DIR } from '../io/files.js';
import { buildGraph, findContradictions, findDanglingRefs, findOrphans, findUnvalidatedAssumptions } from '../graph/graph.js';
import { bold, green, yellow, red, dim, colorId } from '../display/format.js';
import type { Entity, EntityType, RequirementStatus, AssumptionStatus, DecisionStatus } from '../types.js';
...
export function statusCommand(): void {
  ...
  try {
    const config = readFileSync(join(process.cwd(), ARAD_DIR, 'arad.yaml'), 'utf-8');
  ...
  if (unvalidatedBacking.length > 0) {
    console.log(yellow(`  ○ ${unvalidatedBacking.length} unvalidated assumption(s) backing decisions`));
  }
  if (contradictions.length === 0 && danglers.length === 0 && orphans.length === 0) {
    console.log(green('  ✓ No issues'));
  }
  if (unvalidated.length > 0 && unvalidatedBacking.length === 0) {
    console.log(dim(`  ${unvalidated.length} unvalidated assumption(s) (no dependents)`));
  }
}
